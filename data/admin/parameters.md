...
* *Docker* or *Podman* must be installed for start in Linux. В **python_run.sh** имеется параметр `DOCKER_OR_PODMAN`, отвечающий за способ контейниризиции, значение по умолчанию — `podman`.
* The base docker image with Python and required Python packages must be prepared. В **python_run.sh** имя образа прописывается в параметре `IMAGE_NAME`, значение по умолчанию — `python`.
...
* [Virtual environments](https://docs.python.org/3/library/venv.html) can be used when setting *Interpreter path*. Для этого путь интерпретатора должен указывать на python.exe внутри виртуального окружения [подробнее о поиске модулей Python внутри процесса Loginom](#poisk-moduley-python-vnutri-protsessa-loginom). Для корректной работы версия python.exe в виртуальном окружении и версия python3x.dll, через которую выполняется код узла, должны совпадать.

> **Note:** The *Python* node execution is allowed by default in the desktop editions. If required, it can be forbidden.

> **Примечание:** под Linux не поддерживается режим выполнения Python внутри процесса Loginom, следовательно, нет и настройки *Путь библиотеки*. The *Python* nodes configured for execution inside the process will be executed in the separate process in Linux.
