...
* **Minimum weight change threshold**: if the relative change of the weights vector norm is less than the threshold at another training step, the training stops. By default = 0,005.
* **Maximum number of epochs** means the maximum count of the algorithm training iterations. If it is required to limit the training process in time, in this case, it will be stopped upon the set number of epochs even if the training has not reached the optimal point, namely, the minimum weight change threshold has not been reached yet.
...
* **Use a subset of the training set** enables to use a subsample of the training set for autofit.
...
*Figure 1. Auto Selection Algorithm*
